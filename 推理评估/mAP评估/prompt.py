...
def run_success_guaranteed_map(CLASSES):
    ...
    img_infos = coco_gt.loadImgs(img_ids)[:200]
    id = CLASSES_ids[0]
    ...
    #保存预测结果
    pred_file = "glip_success_predictions.json"
    ...
    if predictions:
        ...
        with open('prompt.txt', 'a', encoding='utf-8') as f:
            f.write(f'类别：{CLASSES} , mAP结果：{cocoEval.stats[1]*100:.5f}\n')
    else:
        print("无预测结果")
...
if __name__ == '__main__':
    for CLASSES in List:
        run_success_guaranteed_map(CLASSES)
