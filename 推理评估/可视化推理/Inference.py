...
def draw_images(image, boxes, classes, scores, colors, xyxy=True):
    ...
    # 设置字体,pillow 绘图环节
    font = ImageFont.truetype(font="C:/Windows/Fonts/arial.ttf",size=np.floor(3e-2 * image.size[1] + 0.5).astype('int32'))
...
def glip_inference(image_, caption_):
    ...
    top_preds = glip_demo._post_process(preds, threshold=0.45)
