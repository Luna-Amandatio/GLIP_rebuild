...
class CLIPRerank:
    ...
    Methods:
        glip_load  : 负责glip模型加载与编译
        clip_load  : 负责clip模型加载
    ...
    def glip_load(self):
        """加载GLIP模型"""
        with Timer("GLIP模型加载", enable=self.enable_timer):
            # GLIP配置
            ...
            with Timer("glip模型编译", enable=self.enable_timer):
                #pytorch2.0编译加速
    ...
    def clip_load(self):
        ...
        with Timer("CLIP模型加载", enable=self.enable_timer):
            self.clip = CLIPModel.from_pretrained(r"D:\model\CLIP").to(self.device)
            self.processor = CLIPProcessor.from_pretrained(r"D:\model\CLIP", use_fast=True)

        return self.clip,self.processor
    ...
    def apply_nms_post_fusion(self,temp_predictions, iou_threshold=0.5, conf_threshold=0.1):
